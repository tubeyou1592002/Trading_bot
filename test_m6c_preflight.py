...
Note: BUY Capacity via Agah calculatedquantity endpoint
is BLOCKED pending response structure clarification.
See ambiguity report for details.

Account Cash tests:
- valid sufficient cash → pass
...
# =================================================
# BUY Capacity via Agah (BLOCKED — ambiguity)
# =================================================

def test_buy_capacity_ambiguity():
    """
    The calculatedquantity endpoint response structure
    is not documented in the repository. Cannot implement
    BUY Capacity gate without knowing the response field
    that contains the maximum buy quantity.

    This test documents the ambiguity and blocks M6-C
    BUY Capacity implementation until clarified.
    """
    ambiguity = (
        "GET /api/v1/trades/calculatedquantity response "
        "field name for maximum buy quantity is unknown. "
        "Cannot implement BUY Capacity gate without this "
        "information. Architect must provide the response "
        "structure or field name."
    )
    print("BUY CAPACITY AMBIGUITY:", ambiguity)
    # This test always passes but documents the blocker
    assert True
...
def main():
    ...
    print(
        f"\nM6-C Account Cash tests passed. "
        f"BUY Capacity blocked pending ambiguity resolution. "
        f"({len(tests)} tests)"
    )
