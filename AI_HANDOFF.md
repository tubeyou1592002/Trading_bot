...
## 9. User Application / UI Architecture & Roadmap

### 1. هدف User Application

User Application نرم‌افزار اصلی کاربر نهایی است و برای انجام عملیات معاملاتی روزانه استفاده می‌شود.

UI فقط رابط کاربر است و نباید منطق معاملاتی جدید ایجاد کند.

مسیر اجرای سفارش موجود باید حفظ شود و UI از Core فعلی استفاده کند.

### 2. صفحه اصلی User Application

صفحه اصلی شامل این بخش‌ها است:

#### انتخاب/ورود به حساب

* انتخاب حساب فعال کاربر
* نمایش حساب و کارگزاری فعال
...
* کارمزد
* محاسبه مبلغ نهایی
* امکان استفاده از ماشین‌حساب متناسب با کارگزاری
...
* زمان پایان
* فاصله بین ارسال‌ها
* سایر تنظیمات اجرایی که در آینده به قرارداد اضافه می‌شوند

#### صف سفارش‌ها

* کاربر بتواند سفارش هر حساب را جداگانه آماده و وارد کند.
* حساب‌ها و سفارش‌های آنها باید مستقل باقی بمانند و هویت Account/Broker حفظ شود.

#### TEST

* دکمه Test یک بار سفارش‌های آماده شده را برای تست اجرا می‌کند.
* نتیجه هر سفارش در لاگ صفحه نمایش داده می‌شود.
* Test نباید به معنی ارسال واقعی سفارش باشد.

#### اعمال زمان‌بندی

* پس از آماده شدن سفارش‌های همه حساب‌ها، کاربر با Apply Schedule اجرای زمان‌بندی‌شده را فعال می‌کند.
* پس از شروع Countdown:
  - تایمر معکوس نمایش داده شود.
  - کنترل‌های مربوط به تغییر سفارش غیرفعال شوند.
  - اجرای سفارش‌ها از مسیر Core موجود انجام شود.

#### جدول لاگ کاربر

* لاگ‌های قابل‌فهم و مورد نیاز کاربر مانند:
  - زمان
  - حساب
  - نماد
  - عملیات
  - وضعیت
  - توضیح
* جزئیات فنی Core در UI عادی کاربر نمایش داده نشود.

### 3. صفحه مدیریت حساب‌ها و بروکرها

کاربر بتواند:

* حساب‌های کارگزاری خود را اضافه کند.
* حساب‌های خود را مشاهده کند.
* کاربر بتواند حساب موردنظر را انتخاب کند و وارد آن شود.
* پس از انتخاب Account به صفحه اصلی برگردد.

کاربر فقط Account‌های متعلق به خودش را می‌بیند.

### 4. فعال‌سازی اولیه

در اولین اجرای User Application روی هر سیستم، قبل از دسترسی به صفحه اصلی، فعال‌سازی سیستم باید انجام شود.

مدیریت کامل License و Activation در آینده در Central Server / Admin Panel خواهد شد.

### 5. اطلاعات فنی که نباید در UI عادی کاربر نمایش داده شوند
...
* Trace ID
* جزئیات API بروکر
* جزئیات داخلی Latency
* Simulation Harness
* جزئیات داخلی اجرای Core

نباید بخشی از UI عادی User Application باشند.

### 6. جداسازی Admin Panel

Admin Panel بخشی از User Application نیست.

Admin Panel در آینده به‌صورت یک پنل مستقل برای مدیریت کل سیستم ساخته خواهد شد.

مطالب آینده Admin شامل:

* مدیریت کاربران
* مدیریت License
* مدیریت Activation و دستگاه‌ها
* مشاهده لاگ کاربران
* مشاهده سفارش‌ها و Executionها
* مشاهده وضعیت سیستم‌ها
* سایر امکانات مدیریتی

### 7. Central Server

Central Server در نسخه فعلی User Application پیاده‌سازی نمی‌شود.

در معماری آینده برای مواردی مانند:
...
User Application در آینده اطلاعات لازم را به Central Server ارسال خواهد کرد تا Admin بتواند وضعیت کاربران را از راه دور مشاهده کند.

تا قبل از آن، توسعه و تست User Application می‌تواند بدون VPS و بدون Central Server انجام شود.
...
**Status:** IMPLEMENTED — reviewed and approved; committed as d73f3e0
**Commit:** `d73f3e0`
...
Next step: UI-2 — Account & Broker Management
...
4. طراحی و پیاده‌سازی Admin Panel
5. تکمیل License / Activation مرکزی
6. تست چندکاربره
